...
import { filterSessions, parseSessionSearchQuery } from "../../src/components/selectors/session-selector-search.ts";
import { ApprovalSelectorComponent } from "../../src/components/selectors/approval-selector.ts";
import { ClarificationSelectorComponent } from "../../src/components/selectors/clarification-selector.ts";
...
import { GatewayRequestError } from "../../src/transport/gateway-client.ts";
...
test("mycli shell renders semantic error and warning notices", () => {
	...
	assert.ok(outputLines.includes(
		`   ${glyphs.branch} Upstream closed the response stream before completion.`,
	));
	assert.ok(outputLines.includes(`   ${glyphs.branch} Retry or choose another provider.`));
...
test("mycli shell keeps running subagents out of the transcript and exposes tasks entry", () => {
	...
	assert.doesNotMatch(output, /⎿ Done/);
	assert.match(output, /◇ 1 local agent · \/agents view/);
});
...
test("mycli shell keeps completed background subagents out of the main transcript", () => {
	...
	assert.doesNotMatch(output, /Agent finished/);
	assert.doesNotMatch(output, /\/agents view/);
});
...
test("mycli shell x stops the selected running background subagent", async () => {
	...
	assert.deepEqual(commands, ["/agents kill child-session-bg"]);
	const output = stripAnsi(runtime.ui.render(100).join("\n"));
...
test("mycli shell renders transcript blocks in event order", () => {
	...
	const toolIndex = output.indexOf("• Explored");
...
test("mycli shell renders tools marked hidden by legacy session state", () => {
	...
	assert.match(output, /• Explored/);
	assert.match(output, /Read legacy\.txt/);
});
...
test("mycli shell groups consecutive context tool calls like Codex", () => {
	const state: MycliShellState = {
		...sampleState(),
		messages: [],
		tools: [],
	...
	assert.match(output, /^• Explored/m);
	assert.match(output, /└ Read app\.py/);
	assert.match(output, /Search TODO/);
	assert.match(output, /Search \*\*\/\*\.py/);
...
test("collapsed Read groups preview multiple file targets", () => {
	...
	const output = stripAnsi(renderMycliShell(state, 100).join("\n"));
	assert.match(output, /^• Explored\s*$/m);
	assert.match(output, /^  └ Read session\.ts, token\.ts, provider\.ts, worker\.ts\s*$/m);
	assert.doesNotMatch(output, /more|to expand/);
...
test("mycli shell separates recognized exploration from other Shell commands", () => {
	const state: MycliShellState = {
		...sampleState(),
		messages: [],
		tools: [],
	...
	assert.match(output, /• Explored/);
	assert.match(output, /Search mycli in src/);
	assert.doesNotMatch(output, /src\/mycli\/app\.py/);
...
test("mycli shell labels running and failed collapsed context groups", () => {
	...
	assert.match(runningOutput, /• Exploring/);
	...
	assert.match(failedOutput, /• Explored · 1 failed/);
	assert.match(failedOutput, /Search TODO \(failed\)/);
});
...
test("mycli shell does not collapse context tools across mutating tools", () => {
	...
	assert.match(output, /^  └ Read app\.py/m);
	assert.match(output, /^• Write/m);
	assert.match(output, /^  └ Search TODO/m);
	assert.equal(output.match(/• Explored/g)?.length, 2);
});
...
test("footer separates model and context usage from workspace and session", () => {
	...
	assert.match(output, /deepseek-v4-flash/);
	assert.match(stripAnsi(lines[1] ?? ""), /^ ~[\\/]Desktop[\\/]mycli[\\/]\.worktrees[\\/]mycli-termcn-tui-polish/);
	assert.match(stripAnsi(lines[0] ?? ""), /^ deepseek-v4-flash · medium +11\.3% ctx$/);
	assert.equal(visibleWidth(lines[0] ?? ""), 118);
...
test("footer statusbar modes preserve hierarchy across terminal widths", () => {
	...
	assert.match(stripAnsi(compact[0] ?? ""), /gpt-5\.6-sol.*78\.4% ctx/);
	assert.doesNotMatch(stripAnsi(compact.join("\n")), /Running|优化 TUI 显示/);
	assert.match(stripAnsi(full.join("\n")), /优化 TUI 显示/);
...
test("footer preserves trust and mode without repeating work or permanent hints", () => {
	...
	assert.doesNotMatch(output, /Running|background terminals/);
});
...
test("footer drops git branch before session title on narrow terminals", () => {
	...
	const narrow = new FooterComponent(data, {}).render(48);
	const wide = stripAnsi(new FooterComponent(data, {}).render(160).join("\n"));
...
test("footer keeps compact shape width safe", () => {
	...
	assert.doesNotMatch(output, /status with control chars/);
	assert.doesNotMatch(output, /steer 1|follow-up 1|CH88\.8|\$0\.123/);
	for (const line of lines) {
...
test("work summary omits task counters without reserving empty rows", () => {
	...
	for (const width of [24, 80, 160]) assert.deepEqual(work.render(width), []);
});

test("work summary does not repeat the live activity label", () => {
	const output = stripAnsi(new WorkStatusComponent({
		cwd: "/repo",
	...
	assert.doesNotMatch(output, /Running|Tasks/);
	assert.match(output, /1 shell.*\/ps/);
});
...
test("background work summary uses singular plural and hides zero", () => {
	const one = stripAnsi(new WorkStatusComponent({ cwd: "/repo", backgroundShellCount: 1 }).render(120).join("\n"));
	const two = stripAnsi(new WorkStatusComponent({ cwd: "/repo", backgroundShellCount: 2 }).render(120).join("\n"));
	const zero = stripAnsi(new WorkStatusComponent({ cwd: "/repo", backgroundShellCount: 0 }).render(120).join("\n"));

	assert.match(one, /1 shell.*\/ps/);
	assert.match(two, /2 shells.*\/ps/);
...
test("Read exploration shows only the filename while expanded details retain summaries", () => {
	...
	assert.match(successOutput, /^• Explored\s*$/m);
	assert.match(successOutput, /^  └ Read session\.ts\s*$/m);
	assert.doesNotMatch(successOutput, /186 lines/);
	assert.equal(successOutput.match(/session\.ts/g)?.length, 1);

	const runningOutput = stripAnsi(running.render(80).join("\n"));
	assert.match(runningOutput, /^• Exploring\s*$/m);
	assert.match(runningOutput, /^  └ Read session\.ts\s*$/m);

	const failedOutput = stripAnsi(failed.render(80).join("\n"));
	assert.match(failedOutput, /^• Explored · 1 failed\s*$/m);
	assert.match(failedOutput, /^  └ Read missing\.ts \(failed\)\s*$/m);
	assert.equal(failedOutput.match(/missing\.ts/g)?.length, 1);
...
test("Read titles retain the basename on narrow terminals", () => {
	...
	for (const width of [24, 32, 40]) {
		...
		assert.match(lines.map((line) => stripAnsi(line).trim()).join(""), /session-controller\.ts/);
		for (const line of lines) {
			assert.ok(visibleWidth(line) <= width, `line too wide at ${width}: ${stripAnsi(line)}`);
		}
	}
	...
	for (const width of [24, 32]) {
		...
		assert.match(lines.map((line) => stripAnsi(line).trim()).join(""), /会话控制器\.ts/);
		for (const line of lines) {
...
test("expanded tool rendering keeps display summary and detail separate", () => {
	const rendered = new ToolExecutionComponent({
		id: "grep",
		name: "Grep",
		args: "src: ToolResult",
	...
	assert.match(output, /Search src: ToolResult/);
	assert.match(output, /⎿ 12 matches/);
...
test("trust selector owns keyboard selection", () => {
	...
	assert.match(output, /› 1\. Trust/);

	selector.handleInput("j");
	output = stripAnsi(selector.render(80).join("\n"));
	assert.match(output, /› 2\. Do not trust/);
...
test("mycli shell runtime assembles mounted containers", () => {
	...
	assert.equal(runtime.ui.children[1], runtime.workStatusContainer);
	assert.equal(runtime.ui.children[2], runtime.pendingMessagesContainer);
	assert.equal(runtime.ui.children[3], runtime.subagentTaskContainer);
...
test("mycli shell runtime keeps footer actions hidden across turn and queue state", () => {
	...
	assert.doesNotMatch(output, /enter (?:send|steer)|tab follow-up|esc interrupt|ctrl\+p commands/);
	assert.match(stripAnsi(runtime.statusContainer.render(180).join("\n")), /esc to interrupt/);
...
test("mycli shell runtime updates transcript tool components in place", () => {
	...
	assert.doesNotMatch(output, /3 lines/);
...
test("mycli shell runtime keeps working status in the composer status area", async () => {
	const terminal = new TestTerminal();
	let now = 10_000;
	const runtime = new MycliShellRuntime({
	...
	assert.ok(stripAnsi(runtime.statusContainer.children[0]?.render(100).join("\n") ?? "").includes(workingLabel));
	for (const width of [40, 80, 100]) {
		const lines = [...runtime.chatContainer.render(width), ...runtime.statusContainer.render(width)].map(stripAnsi);
	...
	assert.doesNotMatch(output, /(?:Worked for|Finished in|Completed in|Took) \d/u);
	assert.doesNotMatch(output, /esc to interrupt/);
...
test("working status adopts Codex-style phase headers", () => {
	...
	output = stripAnsi(runtime.statusContainer.render(100).join("\n"));
	assert.ok(output.includes(`Compressing context (0s ${uiGlyphs().bullet} esc to interrupt)`));
	assert.match(output, /Compaction in progress/);
});

test("working status retains its component when transcript output is appended", () => {
	const runtime = new MycliShellRuntime({
		initialState: {
			...sampleState(),
	...
	const firstActivity = runtime.statusContainer.children[0];
	assert.ok(firstActivity);
	assert.match(stripAnsi(runtime.statusContainer.render(100).join("\n")), /esc to interrupt/);
	...
	assert.equal(runtime.statusContainer.children[0], firstActivity);
	const rendered = stripAnsi(runtime.ui.render(100).join("\n"));
...
test("mycli shell opens plan implementation only for a live proposed-plan event", async () => {
	...
	assert.match(output, /enter confirm\s+esc back/);
});
...
test("plan implementation selector stays width safe on narrow terminals", () => {
	...
	for (const width of [32, 40, 80]) {
		...
		if (width === 40) {
			assert.match(stripAnsi(lines.join("\n")), /^ {4}Switch to Default and start coding\.\s*$/m);
		}
	}
});
...
test("mycli shell approval selector replaces editor and submits selected choice", async () => {
	...
	assert.match(output, /↑↓ select\s+enter confirm\s+esc reject\s+ctrl\+a view all/);
...
test("route login cancellation exposes the provider list even for a single missing route", async (t) => {
	const terminal = new TestTerminal();
	const runtime = new MycliShellRuntime({
		initialState: { ...sampleState(), models: [], authProviders: [{ id: "openai", name: "OpenAI", configured: true, authRef: "openai" }] },
		terminal,
		onProviderLoad: async () => [{
			id: "openai", name: "OpenAI", authRef: "work-account", configured: true,
			ready: false, current: true, activation: "active", protocols: ["responses"],
		}],
		onModelLoad: async () => { assert.fail("Missing credentials must not load models."); },
	});
	...
	assert.match(output, /login required/);
	assert.doesNotMatch(output, /Select provider to configure|Login to/);
	terminal.input?.("\x1b");
	assert.equal(runtime.editorContainer.children[0], runtime.editor);
	await runtime.handleClientAction("open_login", "");
	output = stripAnsi(runtime.ui.render(100).join("\n"));
	assert.match(output, /OpenAI.*login required/);
	assert.doesNotMatch(output, /OpenAI.*ready/);
});

test("login uses the returned credential snapshot without marking other refs ready", async (t) => {
	const terminal = new TestTerminal();
	const providers = [{ id: "qwen", name: "Qwen", configured: false, authRef: "qwen-work", credentialSource: "missing" as const }];
	const runtime = new MycliShellRuntime({
		initialState: { ...sampleState(), models: [], authProviders: providers },
		terminal,
		onApiKeyLogin: async () => ({ authProviders: providers }),
	});
	t.after(() => runtime.shutdown());
	runtime.start();
	runtime.showLoginFlow("qwen");
	terminal.input?.("\x1b[200~synthetic-key\x1b[201~");
	terminal.input?.("\r");
	await setTimeout(25);
	assert.deepEqual(runtime.getState().authProviders, providers);
	assert.equal(runtime.getState().authProviders?.[0]?.configured, false);
	await runtime.handleClientAction("open_login", "");
	assert.match(stripAnsi(runtime.ui.render(100).join("\n")), /Qwen.*login required/);
});
...
test("mycli shell login flow replaces editor with auth selectors", async () => {
	...
	assert.match(output, /OpenAI • login required/);
	assert.match(output, /DeepSeek • login required/);
	...
	output = stripAnsi(runtime.ui.render(100).join("\n"));
	assert.match(output, /DeepSeek • login required/);
	assert.doesNotMatch(output, /OpenAI • login required/);
	...
	const runtime = new MycliShellRuntime({
		...
		onApiKeyLogin: async (providerId, apiKey, authRef) => {
			...
			return {
				...
				authProviders: [{ id: providerId, name: "OpenAI", configured: true, authRef, credentialSource: "stored" }],
				authReadiness: { ready: true, providerId, authRef: authRef!, source: "stored" },
			};
		},
	});
...
test("startup credential save failures remain visible and keep login mounted", async () => {
	...
	assert.notEqual(runtime.editorContainer.children[0], runtime.editor);
	assert.equal(runtime.getState().authProviders?.[0]?.configured, false);
	assert.equal(runtime.getState().authReadiness?.ready, false);
});
...
test("forced no-color wins while reduced motion follows the active session", async () => {
	...
	let activity = runtime.statusContainer.render(100).join("\n").trimStart();
	...
	activity = stripAnsi(runtime.statusContainer.render(100).join("\n")).trimStart();
...
test("transcript viewer uses the configured shortcut for both opening and closing", async (t) => {
	const terminal = new TestTerminal();
	const runtime = new MycliShellRuntime({
		initialState: { ...sampleState(), keymap: keymapFixture({ "app.transcript.open": ["ctrl+y"] }) }, terminal,
	});
	...
	terminal.input?.("\x14");
	assert.equal(runtime.ui.hasOverlay(), true, "the old shortcut is no longer the transcript toggle");
	terminal.input?.("\x19");
	assert.equal(runtime.ui.hasOverlay(), false);
});

test("transcript viewer immediately expands retained Shell output", async (t) => {
	const terminal = new TestTerminal();
	const bash = {
	...
	assert.match(stripAnsi(terminal.output), /saved tail/u);
	assert.match(stripAnsi(terminal.output), /Output truncated/u);
});

test("transcript viewer follows retained output while a Shell is still running", async (t) => {
	const terminal = new TestTerminal();
	...
	terminal.input?.("\x14");
	await setTimeout(50);
	assert.match(stripAnsi(terminal.output), /first live chunk/u);
...
test("mycli shell renders reconnect details and keeps the turn interruptible", async () => {
	...
	const runtime = new MycliShellRuntime({
		initialState: {
			...
			footer: {
				...
				liveStateDetail: "upstream request failed",
			},
		},
		terminal,
		onInterrupt: () => {
	...
	assert.match(output.replace(/\s+/g, " "), /upstream request failed/);
	const retryLines = lines.filter((line) => /Reconnecting|upstream request failed/.test(stripAnsi(line)));
...
test("upstream retry detail is transient and terminal detail renders once live and resumed", () => {
	let state = reduceRuntimeEvent(initialRuntimeState(), "turn.started", { turn_id: "turn-upstream", client_turn_id: "client-upstream" });
	state = reduceRuntimeEvent(state, "status.update", { state: "running", kind: "thinking", text: "Thinking" });
	for (let attempt = 1; attempt <= 2; attempt += 1) {
		state = reduceRuntimeEvent(state, "stream.retrying", {
			text: `Reconnecting... ${attempt}/2`, additional_details: "upstream request failed", attempt, max_retries: 2,
		});
		assert.equal(state.transcript.length, 0);
		const output = stripAnsi(renderMycliShell(projectRuntimeState(state), 100).join("\n"));
		assert.equal(projectRuntimeState(state).footer.liveStateDetail, "upstream request failed");
		assert.match(output, new RegExp(`Reconnecting\\.\\.\\. ${attempt}/2`, "u"));
	}
	state = reduceRuntimeEvent(state, "stream.recovered", {});
	assert.equal(projectRuntimeState(state).footer.liveState, "Thinking");
	assert.doesNotMatch(stripAnsi(renderMycliShell(projectRuntimeState(state), 100).join("\n")), /Reconnecting|upstream request failed/u);
	state = reduceRuntimeEvent(state, "turn.failed", {
		turn_id: "turn-upstream", client_turn_id: "client-upstream", code: "retry_exhausted",
		message: "provider retry budget exhausted", additional_details: "upstream request failed (request id: req-test)",
	});
	const resumed = runtimeStateFromTranscript(initialRuntimeState(), {
		items: [{
			id: turnFailedNoticeId("turn-upstream"), type: "error", text: turnFailureNotice("retry_exhausted"), folded: false,
			metadata: { event_kind: "turn_failed", failed_turn_id: "turn-upstream", status: "failed", source: "runtime",
				code: "retry_exhausted", additional_details: "upstream request failed (request id: req-test)" },
		}],
	});
	assert.deepEqual(projectRuntimeState(state).messages, projectRuntimeState(resumed).messages);
	for (const snapshot of [state, resumed]) {
		for (const width of [60, 100]) {
			const lines = renderMycliShell(projectRuntimeState(snapshot), width);
			const output = stripAnsi(lines.join("\n"));
			assert.equal(output.match(/Provider retry budget exhausted\./gu)?.length, 1);
			assert.equal(output.match(/upstream request failed/gu)?.length, 1);
			assert.doesNotMatch(output, /Reconnecting/u);
...
test("mycli shell forwards backend slash commands instead of chatting them", async () => {
	...
	const runtime = new MycliShellRuntime({
		initialState: sampleState(),
		terminal: new TestTerminal(),
		commandNames: ["/changes", "/agents", "/trace"],
		onSubmit: (text) => {
			submitted.push(text);
		},
		onCommandSubmit: (command) => {
	...
	await runtime.editor.onSubmit?.("/agents child-session");
	...
	assert.deepEqual(commands, ["/changes", "/agents child-session", "/trace export"]);
});
...
test("mycli shell command palette includes backend-supported commands", async () => {
	...
	const runtime = new MycliShellRuntime({
		...
		terminal,
		commands: [
			slashCommand("agents", "/agents", "Inspect agents"),
	...
	await assertCommandVisible(/\/agents/);
...
test("alternate-screen resize emits exactly one bounded terminal frame", async () => {
	...
	const assertBoundedFrame = () => {
		...
		for (const row of rows) {
			assert.ok(visibleWidth(stripAnsi(row)) < terminal.columns, `frame row reached wrap column ${terminal.columns}: ${JSON.stringify(stripAnsi(row))}`);
		}
	};
...
test("gateway resolves interrupted inputs after terminal confirmation and durable queue cleanup", () => {
	const source = readFileSync(new URL("../../src/application/gateway-session.ts", import.meta.url), "utf8");
