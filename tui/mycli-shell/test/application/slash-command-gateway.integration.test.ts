...
test("settings load failures remain visible while the gateway remains usable", { timeout: 20_000 }, async () => {
	const result = await promisify(execFile)(process.execPath, [
		"--conditions=mycli-source", "--import", "tsx", "--experimental-test-module-mocks",
		fileURLToPath(new URL("../fixtures/slash-command-gateway.ts", import.meta.url)),
	], { timeout: 15_000, maxBuffer: 1024 * 1024, env: { ...process.env, MYCLI_TUI_NATIVE: "0", MYCLI_TEST_SETTINGS_ERROR: "1" } });
	assert.match(result.stdout, /slash-command-gateway: passed/);
});
