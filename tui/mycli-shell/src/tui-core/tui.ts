...
export class TUI extends Container {
	...
	/** Restores terminal ownership before handing a fatal render error to the process boundary. */
	public onFatalError?: (error: unknown) => void;
	...
	private hardwareCursorCol = 0;
	private hardwareCursorVisible = false;
	private hardwareCursorPositionKnown = false;
	...
	insertHistoryBeforeNextFrame(
		lines: string[],
		options: { clearViewport?: boolean; replaceScrollback?: boolean } = {},
	): void {
		this.pendingHistoryLines = options.replaceScrollback
			? [...lines]
			: [...(this.pendingHistoryLines ?? []), ...lines];
		this.pendingHistoryClearsViewport ||= options.clearViewport === true;
		this.pendingHistoryReplacesScrollback ||= options.replaceScrollback === true;
		this.requestRender();
	}
	...
	private renderHistoryAndFrame(
		...
		if (replaceScrollback) {
			buffer += this.deleteKittyImages(this.previousKittyImageIds);
			buffer += "\x1b[r\x1b[0m\x1b[H\x1b[2J\x1b[3J\x1b[H";
		...
		this.commitHardwareCursorUpdate(cursorUpdate);
	...
	private doRender(): void {
		...
		const pendingHistoryReplacesScrollback = this.pendingHistoryReplacesScrollback;
		this.pendingHistoryLines = null;
		this.pendingHistoryClearsViewport = false;
		this.pendingHistoryReplacesScrollback = false;
		if (pendingHistoryLines && this.terminal.nativeScrollback) {
			this.renderHistoryAndFrame(
				...
				pendingHistoryReplacesScrollback,
				prevViewportTop,
				hardwareCursorRow,
			);
			return;
		...
		const logRedraw = (reason: string): void => {
			...
			const logPath = path.join(os.homedir(), ".mycli", "logs", "tui-debug.log");
		...
		// Configurable via setClearOnShrink() or MYCLI_TUI_CLEAR_ON_SHRINK=0 env var
	...
	private positionHardwareCursor(cursorPos: { row: number; col: number } | null, totalLines: number): void {
		if (!cursorPos || totalLines <= 0) {
			...
			if (
				this.hardwareCursorPositionKnown &&
				targetRow === this.hardwareCursorRow &&
				targetCol === this.hardwareCursorCol &&
				this.showHardwareCursor === this.hardwareCursorVisible
			) {
				return;
			}
		}
	...
	private commitHardwareCursorUpdate(update: HardwareCursorUpdate): void {
		this.hardwareCursorRow = update.row;
		this.hardwareCursorCol = update.col;
		this.hardwareCursorVisible = update.visible;
		this.hardwareCursorPositionKnown = update.positionKnown;
	}
	...
	private buildHardwareCursorUpdate(
		...
		totalLines: number,
		fromRow: number,
	): HardwareCursorUpdate {
		if (!cursorPos || totalLines <= 0) {
			return {
				sequence: "\x1b[?25l",
				row: fromRow,
				col: this.hardwareCursorCol,
				visible: false,
				positionKnown: false,
			};
		}
