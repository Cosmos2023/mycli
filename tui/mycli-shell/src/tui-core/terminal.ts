...
import { OutputBackpressureTracker } from "./output-backpressure.ts";
...
export interface Terminal {
	...
	// Optional Writable-style backpressure signal used to pause frame generation.
...
export class ProcessTerminal implements Terminal {
	...
	stop(): void {
		if (this.clearProgressInterval()) {
			this.write(TERMINAL_PROGRESS_CLEAR_SEQUENCE);
		}
	...
	write(data: string): void {
		this.outputBackpressure.observeWrite(process.stdout.write(data));
	...
	setProgress(active: boolean): void {
		if (active) {
			...
			this.write(TERMINAL_PROGRESS_ACTIVE_SEQUENCE);
