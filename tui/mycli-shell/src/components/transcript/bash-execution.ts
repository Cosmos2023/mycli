...
import { cachedShellSearchPreview, shellSearchHasNoMatches } from "../../transcript/shell-search-preview.ts";
...
export class BashExecutionComponent extends Container {
	...
	override getRenderCacheKey(): number | undefined {
		const showsElapsedTime = this.bash.status === "running"
			&& (this.bash.expanded || !cachedShellSearchPreview(this.bash));
		return showsElapsedTime ? undefined : super.getRenderCacheKey();
	}

	private rebuild(): void {
		this.clear();
		const activity = this.bash.expanded ? undefined : shellContextActivity(this.bash);
		if (activity) {
			this.addChild(new ExplorationSummaryComponent([activity]));
			return;
		}
		const outputLines = terminalContent(this.bash.outputPreview ?? "").split("\n");
		const hasContent = (line: string): boolean => visibleWidth(line.replace(/\s/gu, "")) > 0;
		const first = outputLines.findIndex(hasContent);
		const last = outputLines.findLastIndex(hasContent);
		this.outputLines = first < 0 ? [] : outputLines.slice(first, last + 1);
	...
	private outputComponent(): Text | { render: (width: number) => string[]; invalidate: () => void } {
		if (this.outputLines.length === 0) {
			...
			const omission = !this.bash.expanded && this.bash.status !== "running" && hiddenCount > 0
				? theme.fg("muted", this.indentedHiddenLinesText(hiddenCount))
				: "";
			return new Text(omission, SHELL_CELL_PADDING_X, 0);
		}
		if (this.bash.expanded) {
			return {
	...
	private statusLines(width: number): string[] {
		if (this.bash.status === "running") {
			...
			return commandDisplayLines(this.bash.command, prefix, this.runningSuffix(), width);
		}
	...
	private runningSuffix(): string {
		const details: string[] = [];
		const elapsed = this.elapsedSeconds();
		if (elapsed !== undefined) details.push(`${elapsed}s`);
		if (this.bash.background !== true && this.bash.yielded !== true) details.push("esc to interrupt");
		return details.length > 0 ? ` (${details.join(` ${uiGlyphs().separator} `)})` : "";
	}
	...
	private terminalDetail(): string | undefined {
		...
		const search = cachedShellSearchPreview(this.bash);
		if (shellSearchHasNoMatches(this.bash, search)) details.push(search?.kind === "list" ? "No files found" : "No results");
	...
	private commandDetail(): string {
		const commandLines = terminalContent(this.bash.command).split("\n");
		return [
...
function commandDisplayLines(command: string, headerPrefix: string, suffix: string, width: number): string[] {
	const logicalLines = terminalContent(command)
		.split(/\r?\n/)
