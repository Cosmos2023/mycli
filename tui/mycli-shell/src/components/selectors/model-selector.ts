...
export class ModelSelectorComponent extends Container implements Focusable {
	...
	constructor(options: ModelSelectorOptions) {
		...
		this.maxHeight = options.maxHeight ?? (() => this.tui.terminal.rows);
	...
	refreshProviders(preferredProviderId: string): void {
	...
	override render(width: number): string[] {
		...
		if (this.stage === "provider_loading") {
			...
			lines.push(...gap);
			lines.push(...this.providerRows(safeWidth, listBudget()));
	...
	handleInput(keyData: string): void {
		...
		if (this.maxHeight() < MIN_SELECTOR_HEIGHT) {
	...
	private confirmProvider(): void {
		...
		if (!provider) return;
		if (this.requestLogin(provider)) return;
		void this.loadModels(provider);
	}

	private requestLogin(provider: MycliShellProviderRoute): boolean {
		if (provider.ready || !this.onLoginRequired) return false;
		this.loadGeneration += 1;
		if (this.stage !== "provider") this.searchInput.setValue("");
		this.filterProviders(this.searchInput.getValue());
		this.selectedProviderIndex = Math.max(0, this.filteredProviders.findIndex((entry) => entry.id === provider.id));
		this.stage = "provider";
	...
	private providerRows(width: number, budget: number): string[] {
		if (this.filteredProviders.length === 0) return [theme.fg("muted", "  No matching providers")];
		const maxVisible = Math.min(10, Math.max(1, budget - (this.filteredProviders.length > budget ? 1 : 0)));
		const start = Math.max(0, Math.min(this.selectedProviderIndex - Math.floor((maxVisible - 1) / 2), this.filteredProviders.length - maxVisible));
		...
		if (rows.length < budget && (start > 0 || end < this.filteredProviders.length)) {
			rows.push(this.line(theme.fg("muted", `  ${this.selectedProviderIndex + 1}/${this.filteredProviders.length}`), width));
		}
		return rows;
	}

	private modelRows(width: number, budget: number): string[] {
		if (this.filteredModels.length === 0) return [theme.fg("muted", "  No matching models")];
		const maxVisible = Math.min(10, Math.max(1, budget - (this.filteredModels.length > budget ? 1 : 0)));
	...
	private choiceRows(rows: string[], selectedIndex: number, budget: number): string[] {
		const start = Math.max(0, Math.min(selectedIndex - Math.floor((budget - 1) / 2), rows.length - budget));
