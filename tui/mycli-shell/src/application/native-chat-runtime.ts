...
} from "../components/transcript/transcript-renderer.ts";
import type {
...
import type { MycliUiAction, MycliUiActionDispatcher } from "../interaction/ui-actions.ts";
...
export class NativeChatRuntime {
	...
	constructor(private readonly options: NativeChatRuntimeOptions) {
		...
		this.routingNames = [...(options.commandNames ?? commandRoutingNames(options.commands ?? []))];
		if (!options.streams) {
	...
	start(): void {
		...
		this.readline.on("line", (line) => {
			this.enqueueAction(async () => {
				if (this.started && !this.stopping && this.generation === generation) await this.handleLine(line);
			});
		});
		this.readline.on("SIGINT", () => {
			...
			void this.handleInterrupt()
				.catch((error: unknown) => { this.writeLines([safeErrorMessage(error, "Interrupt failed")]); })
				.finally(() => { this.interruptQueued = false; });
		});
	...
	private async handleLine(line: string): Promise<void> {
		...
		if (isSlashCommandSubmission(text, this.routingNames)) {
	...
	private handleReadlineClose(): void {
		...
		this.started = false;
		void this.runAction({ type: "exit", reason: "normal" }, "Exit failed");
	}
