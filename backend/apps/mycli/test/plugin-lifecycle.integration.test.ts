...
async function fixture(t: TestContext) {
	...
	t.mock.method(McpClient.prototype, "listTools", async function(this: McpClient, activeSignal: AbortSignal) {
		clients.set(this, clients.get(this) ?? version);
	...
	const composition = await createRuntimeIntegrationComposition({
		...
		createSubagentSupervisor: () => ({
			...
			unload: async () => false, list: () => [], recoverLegacyAbandoned: () => 0, close: async () => undefined,
		}),
		resolveSubagentSpawnContext: () => assert.fail("no agents should spawn"),
	});
	t.after(() => composition.close());
	return { composition, packages, clients, closed,
