...
export class ApprovalContinuationCoordinator {
	...
	prepare(input: ApprovalSuspensionInput): PendingApprovalContinuation {
		return pendingFromInput(this.#sessionId, input);
	}

	serialize(
		pending: PendingApprovalContinuation,
		conversation: readonly CanonicalMessage[],
	): Pick<SaveApprovalSuspensionInput, "pendingDecision" | "suspendedTurn"> {
		return {
			pendingDecision: pendingDecisionState(pending),
			suspendedTurn: suspendedTurnState(pending, conversation),
		};
	}
	...
	restore(
		...
		return pendingFromStates(this.#sessionId, pending, suspended);
	}

	async authorize(pending: PendingApprovalContinuation, choice: ApprovalChoice): Promise<void> {
		...
		if (choice === "always_allow") {
			...
			this.#publishExecPolicyRules(await this.#ruleStore.load());
		}
		if (choice === "allow_session" && !pending.permissionRequest) {
...
export function rejectedResult(call: CanonicalToolCall): ToolExecutionResult {
	return Object.freeze({
		callId: call.callId,
		toolName: call.name,
		success: false,
