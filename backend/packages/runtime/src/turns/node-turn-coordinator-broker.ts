...
import { modelInputSha256, normalizeCanonicalImages } from "@mycli/core";
import type {
...
export interface NodeTurnCoordinatorBrokerOptions {
	...
	readonly attemptLedger?: ProviderAttemptLedgerStore;
...
export class NodeTurnCoordinatorBroker {
	...
	recordProviderAttempt(input: {
		...
		if (!this.#options.attemptLedger) throw new StorageFailure("provider attempt ledger is unavailable");
		return this.#options.attemptLedger.append({ ...input, sessionId: this.#options.sessionId });
	}
...
function storedToolResult(
	...
	const metadata = { ...value.metadata as Readonly<Record<string, unknown>> };
	const errorContext = value.success ? undefined : readErrorContext(value.errorContext ?? metadata.error_context);
	...
	return Object.freeze({
		...
		...(value.images === undefined ? {} : { images: normalizeCanonicalImages(value.images) }),
