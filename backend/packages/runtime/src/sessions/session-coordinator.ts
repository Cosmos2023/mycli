...
export interface SessionCoordinatorOptions<Binding> {
	...
	readonly prepare: (sessionId: string, intent: "resume" | "inspect") => PreparedSession<Binding> | Promise<PreparedSession<Binding>>;
...
export class SessionCoordinator<Binding> {
	...
	async inspect(sessionId: string): Promise<PreparedSession<Binding>> {
		...
		return freezePrepared(await this.#prepareSession(normalized, "inspect"));
	}
	...
	async resume(sessionId: string): Promise<ActiveSessionSnapshot<Binding>> {
		...
		try {
			...
			const prepared = freezePrepared(await this.#prepareSession(normalized, "resume"));
