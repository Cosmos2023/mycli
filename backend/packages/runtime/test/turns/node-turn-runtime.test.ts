...
import { ApprovalPolicy } from "../../../tools/src/policy/approval-policy.ts";
import {
	NodeTurnRuntime,
...
test("completion timing separates durable commit, snapshot waiting, and event publication", async () => {
	const trace: string[] = [];
	...
	const running = createRuntime({
		store,
		provider: scriptedProvider(trace, [], [[{ type: "text_delta", text: "done" }, { type: "completed" }]]),
		toolRouter: new SequencedRouter(trace),
		monotonicClock: () => now,
		...
		emitted.push(event);
		if (event.type === "turn_completed") now += 5;
	}, { signal: new AbortController().signal });
	try {
		await snapshotStarted.promise;
		assert.equal(store.loadTurn()?.status, "completed");
		assert.equal(emitted.some((event) => event.type === "turn_completed"), false);
	} finally {
		releaseSnapshot.resolve();
		await running;
	}
	assert.deepEqual(diagnostics.find((event) => event.kind === "turn_completion_diagnostics"), {
		kind: "turn_completion_diagnostics", turnId: "turn-1", commitMs: 0,
		continuationMs: 0, snapshotMs: 3_000, publishMs: 5, elapsedMs: 3_005, snapshotWritten: true,
	});
});
...
test("legacy approval coordinators retain phase barriers and provider result order", async () => {
	const trace: string[] = [];
	const store = new FakeStore(trace);
	const router = new ControlledToolRouter(trace, new Set(["Shell"]));
	const approvals = approvalRuntimeFixture(trace, router, store);
	const running = createRuntime({
		store,
		provider: scriptedProvider(trace, [], [[
			{ type: "tool_call", callId: "allowed-1", name: "Shell", argumentsJson: '{"command":"first"}' },
			{ type: "tool_call", callId: "allowed-2", name: "Shell", argumentsJson: '{"command":"second"}' },
			{ type: "tool_call", callId: "needs-approval", name: "Shell", argumentsJson: '{"command":"third"}' },
			{ type: "completed", responseId: "resp-tools" },
		]]),
		toolRouter: router,
		toolDefinitions: [SHELL_TOOL_DEFINITION],
		approvalCoordinator: approvals.coordinator,
		approvalPolicy: { evaluate: (call) => call.callId === "needs-approval"
			? { kind: "request", callId: call.callId, toolName: call.name, preview: "third", reason: "Requires approval", options: ["approve_once", "reject"] }
			: { kind: "allow", callId: call.callId, toolName: call.name, preview: "Allowed Shell", reason: "Allowed" },
		},
	}).submit(submission(), () => {}, { signal: new AbortController().signal });
	await router.waitForStarted(2);
	assert.equal(approvals.pending(), undefined);
	router.release("allowed-2");
	assert.equal(store.toolResults.length, 0);
	router.release("allowed-1");
	assert.equal((await running).status, "in_progress");
	assert.deepEqual(store.toolResults.map((entry) => entry.result.callId), ["allowed-1", "allowed-2"]);
	assert.equal(approvals.pending()?.decisionId, "needs-approval");
	assert.deepEqual(router.startedCallIds, ["allowed-1", "allowed-2"]);
});
...
test("sequential shell approval publishes the exact command while keeping durable summaries compact", async () => {
	const trace: string[] = [];
	const store = new FakeStore(trace);
	const command = "node <<'SCRIPT'\n  console.log('first  second');\nSCRIPT";
	const justification = "Run the requested multiline diagnostic.";
	const router = new SequencedRouter(trace, new Set());
	const approvals = approvalRuntimeFixture(trace, router, store);
	const emitted: RuntimeEvent[] = [];
	await createRuntime({
		store,
		provider: scriptedProvider(trace, [], [[
			{ type: "tool_call", callId: "call-shell", name: "Shell", argumentsJson: JSON.stringify({ command, justification }) },
			{ type: "completed", responseId: "resp-tools" },
		]]),
		toolRouter: router,
		approvalPolicy: new ApprovalPolicy({ workspaceRoot: "/workspace", shellKind: "posix" }),
		approvalCoordinator: approvals.coordinator,
		toolDefinitions: [SHELL_TOOL_DEFINITION],
	}).submit(submission(), emitted.push.bind(emitted), { signal: new AbortController().signal });
	const request = emitted.find((event) => event.type === "approval_requested");
	assert.ok(request);
	assert.equal(request.commandPreview, command);
	assert.equal(request.commandTruncated, false);
	assert.equal(request.justification, justification);
	assert.equal(approvals.pending()?.preview, "Shell command requires approval");
	assert.equal(router.calls, 0);
});

test("Shell publishes an approval without another model request when its reason is missing", async () => {
	const trace: string[] = [];
	const store = new FakeStore(trace);
	const requests: ProviderRequest[] = [];
	const router = new SequencedRouter(trace, new Set());
	...
	const emitted: RuntimeEvent[] = [];
	const args = {
		command: "git status --short --branch", sandbox_permissions: "require_escalated",
		description: "Inspect the branch and working tree.",
	};
	const result = await createRuntime({
		store,
		provider: scriptedProvider(trace, requests, [[
			{ type: "tool_call", callId: "missing-reason", name: "Shell", argumentsJson: JSON.stringify(args) },
			{ type: "completed", responseId: "response-missing" },
		]]),
		toolRouter: router,
		approvalPolicy: new ApprovalPolicy({ workspaceRoot: "/workspace", shellKind: "posix" }),
		approvalCoordinator: approvals.coordinator,
	...
	assert.equal(requests.length, 1);
	assert.equal(store.toolResults.length, 0);
	assert.deepEqual(emitted.filter((event) => event.type === "approval_requested")
		.map((event) => [event.callId, event.justification]), [["missing-reason", undefined]]);
	assert.equal(approvals.pending()?.call.argumentsJson, JSON.stringify(args));
});
...
test("execution policy configuration updates routine approval profile", () => {
	...
	const shellCall: CanonicalToolCall = {
		callId: "call-shell",
		name: "Shell",
		argumentsJson: JSON.stringify({ command: "rm -rf build", justification: "Remove generated build files." }),
	};
	const runtime = createRuntime({
		store: new FakeStore([]),
		provider: scriptedProvider([], [], []),
