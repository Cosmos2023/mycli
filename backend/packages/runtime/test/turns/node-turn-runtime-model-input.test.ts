...
test("Worker startup failures and queued cancellation have durable terminal records", async (t) => {
	...
	const runtime = new NodeTurnRuntime({
		sessionId: "session-1", workspaceRoot: root, threadId: "session-1", instructions: "Test runtime.",
		store, resolveConfig: () => { assert.fail("startup failure must not prepare provider input"); },
		createProvider: () => { assert.fail("startup failure must not create a provider"); },
		loadLocalImages: () => [], createTurnId: () => "unused", clock: () => new Date().toISOString(),
		publishLifecycle: () => undefined,
	});
	const workerRuntime = new WorkerLeasedRootTurnRuntime({
		sessionId: "session-1", runtime,
		pool: { acquire: async () => { throw new Error("private worker startup detail"); } },
	});
	const events: RuntimeEvent[] = [];
	for (const cancelled of [false, true]) {
		const suffix = cancelled ? "cancelled" : "failed";
		const controller = new AbortController();
		if (cancelled) controller.abort();
		const result = await workerRuntime.submit({
			clientTurnId: `client-${suffix}`, turnId: `turn-${suffix}`, message: "Try a new turn.",
		}, (event) => { events.push(event); }, { signal: controller.signal });
		assert.equal(result.status, cancelled ? "interrupted" : "failed");
		assert.equal(result.error_code, cancelled ? "interrupted" : "config_error");
		assert.deepEqual(store.loadTurn("session-1", `client-${suffix}`), result);
	}
	assert.deepEqual(events.map((event) => event.type), ["turn_failed", "turn_interrupted"]);
	assert.doesNotMatch(JSON.stringify(events), /private worker startup detail/u);
	assert.equal(store.providerAttemptLedger.list({ sessionId: "session-1" }).length, 0);
	store.close();
	const reopened = openRuntimeSessionStore({ dbPath });
	try {
		assert.equal(reopened.loadTurn("session-1", "client-failed")?.status, "failed");
		assert.equal(reopened.loadTurn("session-1", "client-cancelled")?.status, "interrupted");
		assert.doesNotMatch(JSON.stringify(reopened.loadConversationItems("session-1")), /private worker startup detail/u);
	} finally {
		reopened.close();
	}
});
...
test("persists collaboration, permissions, skill catalog, workspace, and environment context", async (t) => {
	...
	const permissionContext = [
		...(requests[0]?.developerInstructions ?? []),
