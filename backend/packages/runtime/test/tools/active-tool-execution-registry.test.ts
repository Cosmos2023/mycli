...
test("active tool registry retains terminal identity through interruption without adding it to diagnostics", () => {
	const events: RuntimeEvent[] = [];
	const diagnostics: RuntimeDiagnosticEvent[] = [];
	const registry = new ActiveToolExecutionRegistry({ recordDiagnostic: (event) => { diagnostics.push(event); } });
	const interaction = { shell_id: "shell-1", kind: "input" as const, input_preview: '"y\\n"' };
	const emit = (event: RuntimeEvent): void => { events.push(event); };
	const claim = registry.begin({ turnId: "turn-1", callId: "input-1", toolName: "WriteStdin",
		interruptErrorKind: "tool_interrupted", terminalInteraction: interaction }, emit);
	assert.deepEqual(events[0], { type: "tool_execution_started", callId: "input-1", toolName: "WriteStdin", terminalInteraction: interaction });
	registry.interrupt(claim, emit);
	const failed = events.find((event) => event.type === "tool_execution_failed");
	assert.deepEqual(failed?.metadata.terminal_interaction, interaction);
	assert.doesNotMatch(JSON.stringify(diagnostics), /input_preview|shell-1/u);
});
