...
test("resource-only MCP servers remain discoverable through the real SDK", { timeout: 10_000 }, async (t) => {
	...
	const client = new McpClient({
		config: { ...stdioConfig(join(root, "unused.pid"), 2_000), env: {}, args: [join(dirname(fixturePath), "mcp-images-server.mjs"), "--resources-only"] },
		cwd: root, sandboxProfile: fullAccessSandbox(root),
	});
	t.after(async () => { await client.close(); await rm(root, { recursive: true, force: true }); });
	assert.deepEqual(await client.listTools(new AbortController().signal), []);
	const resources = await client.listResources(new AbortController().signal);
	assert.deepEqual(resources.map((resource) => resource.uri), ["data:///readme", "data:///image"]);
	assert.equal((await client.readResource("data:///readme", new AbortController().signal))[0]?.text, "Resource text reached the model.");
});
