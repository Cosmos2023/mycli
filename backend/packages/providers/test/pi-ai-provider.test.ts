...
import { toPiAiContext } from "../src/pi-ai-context.ts";
import type { PiAiApi, PiAiModelConfig } from "../src/pi-ai-model.ts";
import { PiAiProvider } from "../src/pi-ai-provider.ts";
import { piAiReplayTransportIdentity } from "../src/pi-ai-replay.ts";
...
test("rejects invalid pi-ai terminal states", async (context) => {
	const cases: Array<{
		...
		},
		{
			...
			pattern: /event after terminal completion/u,
		},
	];
	for (const scenario of cases) {
