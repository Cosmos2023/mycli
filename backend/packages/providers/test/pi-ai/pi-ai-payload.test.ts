...
import {
	...
	contextItem,
	parityRequest,
} from "../support/provider-parity-fixtures.ts";
...
test("preserves view_image detail in real SDK payloads without changing identical untagged images", async () => {
	for (const model of ["gpt-5.4", "gpt-5.2"]) {
		const body = await capturePayload({ ...minimalRequest("openai", "responses"), model, items: [
			{ type: "user", text: "inspect", images: [{ mediaType: "image/png", data: "aW1hZ2U=" }] },
			{ type: "assistant_tool_calls", text: "", calls: [{ callId: "view", name: "view_image", argumentsJson: "{}" }] },
			{ type: "tool_result", callId: "view", toolName: "view_image", output: "Viewed image", success: true,
				images: [{ mediaType: "image/png", data: "aW1hZ2U=", detail: "original" }] },
		], webSearchMode: "live", tools: [{ id: "builtin:view_image", name: "view_image", description: "View a local image.",
			inputSchema: { type: "object", properties: { path: { type: "string" }, detail: { type: "string", enum: ["high", "original"] } }, required: ["path"] } }] });
		assert.equal(records(body.tools).at(-1)?.type, "web_search");
		const parameters = records(body.tools)[0]!.parameters as { properties: Record<string, unknown> };
		assert.equal(Object.hasOwn(parameters.properties, "detail"), model === "gpt-5.4");
		const input = records(body.input);
		const user = input.find((item) => item.role === "user")!;
		assert.equal(records(user.content).find((item) => item.type === "input_image")?.detail, "auto");
		const output = input.find((item) => item.type === "function_call_output")!;
		assert.equal(records(output.output).find((item) => item.type === "input_image")?.detail,
			model === "gpt-5.4" ? "original" : "high");
	}
	const chat = await capturePayload({ ...minimalRequest("compatible", "chat_completions"), items: [
		{ type: "user", text: "inspect", images: [{ mediaType: "image/png", data: "aW1hZ2U=", detail: "high" }] },
	] });
	const content = records(records(chat.messages).find((item) => item.role === "user")!.content);
	assert.deepEqual(content.find((item) => item.type === "image_url")?.image_url,
