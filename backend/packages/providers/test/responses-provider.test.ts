...
test("serializes canonical images as Responses input_image content", async () => {
	const ResponsesProvider = Reflect.get(providers, "ResponsesProvider") as ResponsesProviderConstructor | undefined;
	assert.equal(typeof ResponsesProvider, "function");
	let capturedRequest: Record<string, unknown> | undefined;
	const client: ResponsesClient = {
		create: async (body) => {
			capturedRequest = body;
			return events([completedResponse("resp-image")]);
		},
	};
	...
	await collect(new ResponsesProvider!({ client }).stream({
		...
		items: [{
			type: "user",
			text: "describe",
			images: [{ mediaType: "image/webp", data: "aW1hZ2U=" }],
		}],
	}, { signal: new AbortController().signal }));
...
test("preserves contextual-user and developer authority in Responses input order", async () => {
	const ResponsesProvider = Reflect.get(providers, "ResponsesProvider") as ResponsesProviderConstructor | undefined;
	assert.equal(typeof ResponsesProvider, "function");
	let capturedRequest: Record<string, unknown> | undefined;
	const client: ResponsesClient = {
		create: async (body) => {
			capturedRequest = body;
			return events([completedResponse("resp-authority")]);
		},
	};
...
test("keeps the prior Responses wire input as an exact prefix", async () => {
	const ResponsesProvider = Reflect.get(providers, "ResponsesProvider") as ResponsesProviderConstructor | undefined;
	assert.equal(typeof ResponsesProvider, "function");
	...
	const client: ResponsesClient = {
		create: async (body) => {
			captured.push(body);
			return events([completedResponse(`resp-prefix-${captured.length}`)]);
		},
	};
	const provider = new ResponsesProvider!({ client });
	const firstItems = [
	...
	await collect(provider.stream({
		...request(),
		developerInstructions: ["durable policy"],
		previousResponseId: "resp-prefix-1",
		items: [
			...firstItems,
	...
	const firstInput = captured[0]?.input as readonly unknown[];
	const secondInput = captured[1]?.input as readonly unknown[];
	assert.deepEqual(secondInput.slice(0, firstInput.length), firstInput);
	assert.equal(captured[0]?.prompt_cache_key, "cache-key");
	assert.equal(captured[1]?.prompt_cache_key, "cache-key");
	assert.equal("previous_response_id" in (captured[1] ?? {}), false);
});
