...
import type { ProviderRouteDescriptor } from "../../src/registry/provider-directory-types.ts";
...
test("provider registry rejects missing declared-route credentials before pi-ai construction", () => {
	assert.throws(
		() => new ProviderRegistry().create(config({ apiKey: undefined }), route({ source: "pi_ai_declared" })),
