...
import {
	...
	type ProviderNativeTransportSnapshot,
} from "@mycli/core";
import { ProviderFailure } from "../errors.ts";
import { serializeJsonObject } from "./json-object.ts";
import { invalidPiAiToolArguments } from "./pi-ai-failure.ts";
...
export interface PiAiReplayTransportIdentity {
	...
	readonly nativeTransportSha256?: string;
}
...
export function piAiReplayTransportIdentity(input: {
	...
	return Object.freeze({
		...
		...(input.nativeTransport ? { nativeTransportSha256: modelInputSha256(input.nativeTransport) } : {}),
	});
}
...
function parseReplayTransport(value: unknown): PiAiReplayTransportIdentity | undefined {
	...
	const expectedKeys = [
		...
		...(value.nativeTransportSha256 === undefined ? [] : ["nativeTransportSha256"]),
	].sort();
	...
	if (value.nativeTransportSha256 !== undefined && (typeof value.nativeTransportSha256 !== "string"
		|| !SHA256_PATTERN.test(value.nativeTransportSha256))) return undefined;
	return Object.freeze({
		version: REPLAY_TRANSPORT_VERSION,
		...
		...(typeof value.nativeTransportSha256 === "string" ? { nativeTransportSha256: value.nativeTransportSha256 } : {}),
	});
}
...
function sameReplayTransport(
	...
	return left.version === right.version
		...
		&& left.nativeTransportSha256 === right.nativeTransportSha256;
}
