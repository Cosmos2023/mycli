...
import type { ProviderNativeApi, ProviderNativeTransportSnapshot, ProviderRouteId } from "@mycli/core";
import {
...
const API_FACTORIES: Readonly<Record<CompatiblePiAiApi, () => ProviderStreams>> = Object.freeze({
...
export async function createPiAiSnapshot(config: PiAiModelConfig): Promise<PiAiSnapshot> {
	...
	const catalogModels = builtinProvider === undefined
		...
		: builtinProvider.getModels().filter((model): model is Model<PiAiApi> =>
			model.api === api);
	if (nativeTransport && (!builtinProvider || (nativeTransport.modelSource === "catalog"
		&& !catalogModels.some((model) => model.id === config.model)))) {
...
function catalogPiAiSnapshot(
	...
	const model = catalogued
		...
		: declaredPiAiModel(config, api, config.nativeTransport?.catalogProviderId ?? config.provider);
	...
	const routeProvider = Object.freeze<Provider<PiAiApi>>({
		...
		baseUrl: config.apiBaseUrl,
		auth: provider.auth,
		...(provider.headers ? { headers: Object.freeze({ ...provider.headers }) } : {}),
		getModels: () => routeModels,
		...(provider.filterModels ? { filterModels: (models, credential) =>
			provider.filterModels!(models, credential).filter((candidate): candidate is Model<PiAiApi> => candidate.api === api) } : {}),
...
function cloneCataloguedModel(
	...
	return Object.freeze({
		...model,
		provider: config.nativeTransport?.catalogProviderId ?? config.provider,
		baseUrl: config.apiBaseUrl,
		input,
...
export function piAiRequestModel(
	...
	if (request.protocol !== providerNativeProtocol(snapshot.api)
		|| stableModelInputJson(request.nativeTransport) !== stableModelInputJson(snapshot.nativeTransport)) {
...
export function piAiStreamOptions(
	...
	return {
		...
		...(config.providerEnv || config.nativeTransport?.azure ? { env: {
			...config.providerEnv,
			...(config.nativeTransport?.azure ? {
				AZURE_OPENAI_BASE_URL: config.apiBaseUrl,
				AZURE_OPENAI_API_VERSION: config.nativeTransport.azure.apiVersion,
				AZURE_OPENAI_DEPLOYMENT_NAME_MAP: `${config.model}=${config.nativeTransport.azure.deploymentName}`,
			} : {}),
		} } : {}),
		signal,
		maxRetries: 0,
...
export function piAiApi(protocol: ProviderRequest["protocol"]): CompatiblePiAiApi {
	switch (protocol) {
		case "responses":
...
function validatedNativeTransport(config: PiAiModelConfig): ProviderNativeTransportSnapshot {
	try {
		const native = parseProviderNativeTransportSnapshot(config.nativeTransport);
		if (config.routeSource === "pi_ai_declared"
			|| native.catalogProviderId !== (config.catalogProviderId ?? config.provider)
			|| native.modelId !== config.model || providerNativeProtocol(native.api) !== config.protocol
			|| native.endpointSha256 !== providerNativeEndpointSha256(config.apiBaseUrl)) throw nativeConfigurationFailure();
		return native;
	} catch { throw nativeConfigurationFailure(); }
}

function nativeConfigurationFailure(): ProviderFailure {
	return new ProviderFailure({ code: "config_error", message: "native provider snapshot does not match a supported catalog model" });
}
...
function unsupportedReasoning(effort: ReasoningEffort | undefined): ProviderFailure {
	return new ProviderFailure({
		...
		errorReason: { reason: "capability.reasoning_unsupported", details: { feature: effort ?? "none" } },
		outcome: { state: "not_started", effects: "none" },
	});
}
