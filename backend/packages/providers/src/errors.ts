...
export function classifyProviderError(error: unknown): ProviderFailure {
	...
	const providerErrorCode = safeErrorToken(record.code) ?? safeErrorToken(nested.code);
	const providerErrorType = safeErrorToken(record.type) ?? safeErrorToken(nested.type);
	if (providerErrorCode) diagnostics.provider_error_code = providerErrorCode;
