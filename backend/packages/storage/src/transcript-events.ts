...
export function transcriptEventIdentity(
	event: Pick<TranscriptEventEnvelope, "sessionId" | "eventId">,
): string {
	return stableJson([event.sessionId, event.eventId]);
}
...
function toolResult(value: unknown): ToolResultTranscriptPayload {
	...
	keys(result, ["callId", "toolName", "output", "success"], [
...
function conversationItem(value: unknown, field: string): CanonicalConversationItem {
	...
	if (type === "tool_result") {
		keys(item, ["type", "callId", "toolName", "output", "success"], [
