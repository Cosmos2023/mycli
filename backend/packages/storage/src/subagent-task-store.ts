...
export interface SubagentTaskStore {
	...
	getLatestByChildSession(childSessionId: string): SubagentTaskRecord | undefined;
...
export const SUBAGENT_TASK_DESCRIPTION_MAX_CHARS = 2_048;
	...
	getLatestByChildSession(childSessionId: string): SubagentTaskRecord | undefined {
		return this.#read(() => {
			const row = this.#database.prepare(`
				...
				ORDER BY created_at DESC, task_id DESC
				LIMIT 1
			`).get(identifier(childSessionId, "childSessionId")) as SubagentTaskRow | undefined;
			return row ? recordFromRow(row) : undefined;
		});
	}
...
function validateReserveInput(input: ReserveSubagentTaskInput): ReserveSubagentTaskInput {
	return Object.freeze({
		...
		...(input.mode === undefined ? {} : { mode: subagentMode(input.mode) }),
...
function sameReservation(
	...
	return record.taskId === input.taskId
		...
		&& record.payload.mode === input.mode
		&& record.payload.description === input.description;
}
...
function parsePayload(value: unknown): SubagentTaskPayload {
	...
	return freezePayload({
		...(parsed.mode === undefined ? {} : { mode: subagentMode(parsed.mode) }),
...
function subagentMode(value: unknown): "foreground" | "background" {
	if (value !== "foreground" && value !== "background") {
		throw new StorageFailure("subagent task mode is invalid");
	}
	return value;
}
