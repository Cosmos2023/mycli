...
import { migrateV13ErrorContexts } from "../migrations/v13/v13-error-context-migration.ts";
import { SQLiteProviderAttemptLedger } from "../projections/provider-attempt-ledger.ts";
import type { ProviderAttemptLedgerStore, ProviderAttemptLedgerFailpoint } from "../projections/provider-attempt-ledger.ts";
import { runtimeErrorStopReason } from "../sessions/runtime-error-stop-reason.ts";
import {
...
import { ftsMatchQuery, searchSnippet } from "../sessions/session-search.ts";
import { canonicalConversationItem } from "../projections/legacy-provider-projection.ts";
import { assertImagePathCount, canonicalImages } from "../artifacts/canonical-images.ts";
import {
	parseTranscriptEventAppendInput,
	parseTranscriptEventEnvelope,
	type AppendTranscriptDisplayActivityInput,
	type AppendCompactionActivityInput,
...
} from "../sessions/sqlite-session-state.ts";
import { SQLiteSubagentTaskRepository } from "../agents/subagent-task-store.ts";
import type { SubagentTaskStore } from "../agents/subagent-task-store.ts";
import { SQLiteAgentThreadRepository } from "../agents/agent-thread-store.ts";
import type {
	AgentSpawnStore,
	AgentThreadStore,
} from "../agents/agent-thread-store.ts";
import { SQLiteAgentLifecycleRepository } from "../agents/agent-lifecycle-store.ts";
import type {
	AgentLifecycleFailpoint,
	AgentLifecycleStore,
} from "../agents/agent-lifecycle-store.ts";
import { SQLiteAgentMailboxRepository } from "../agents/agent-mailbox-store.ts";
import type { AgentMailboxStore } from "../agents/agent-mailbox-store.ts";
import { SQLiteModelInputLedger } from "../projections/model-input-ledger.ts";
import type {
	ModelInputLedgerFailpoint,
	ModelInputLedgerStore,
} from "../projections/model-input-ledger.ts";
import { SQLiteAgentEffectLedger } from "../agents/agent-effect-ledger.ts";
import type { AgentEffectLedgerStore } from "../agents/agent-effect-ledger.ts";
import { projectTranscriptEventsToProviderItems } from "../projections/transcript-provider-projector.ts";
import { projectTranscriptEventsToReadableItems } from "../projections/transcript-readable-projector.ts";
import { projectTranscriptEventToSearchDocument } from "../projections/transcript-search-projector.ts";
...
import type { StoredSessionContentBlob } from "../artifacts/session-content-blob.ts";
import {
...
import { SQLiteTurnTerminalizationRepository } from "../sessions/turn-terminalization-repository.ts";
import type { TurnTerminalizationFailpoint } from "../sessions/turn-terminalization-repository.ts";
import type {
...
export interface TranscriptEventRepository extends SessionLeaseStore {
	...
	appendCompactionActivity(input: AppendCompactionActivityInput): boolean;
	...
	interruptSessionForResume(sessionId: string): number;
...
export class SQLiteTranscriptEventRepository implements TranscriptEventRepository {
	...
	constructor(options: SQLiteTranscriptEventRepositoryOptions) {
		...
		try {
			...
			this.errorContextVersion = this.#schemaVersion >= SCHEMA_V14_VERSION ? 1 : undefined;
			...
			this.providerAttemptLedger = new SQLiteProviderAttemptLedger({
				database: this.#database,
				write: <Result>(operation: () => Result) => this.#write(operation),
				manifests: this.modelInputLedger,
				ownerId: this.#ownerId,
				clock: this.#clock,
				available: this.#schemaVersion >= SCHEMA_V13_VERSION,
				errorContextVersion: this.errorContextVersion,
				...(options.providerAttemptFailpoint ? { failpoint: options.providerAttemptFailpoint } : {}),
			});
			this.turnTerminalizations = new SQLiteTurnTerminalizationRepository({
				database: this.#database,
				effectLedger: this.agentEffectLedger,
				providerAttemptLedger: this.providerAttemptLedger,
				write: <Result>(operation: () => Result) => this.#write(operation),
	...
	appendCompactionActivity(input: AppendCompactionActivityInput): boolean {
		return this.#write(() => {
			const checkpoint = this.#stateRepository.loadState(input.activity.sessionId, "compact_checkpoint");
			if (!isRecord(checkpoint) || checkpoint.status !== "in_progress"
				|| checkpoint.window_id !== input.checkpointId
				|| checkpoint.turn_id !== input.activity.turnId
				|| checkpoint.summary_request_fingerprint !== input.fingerprint) return false;
			this.#appendDisplayActivityEvent(input.activity);
			this.#touchExistingSession(input.activity.sessionId, input.activity.createdAt);
			return true;
		});
	}
	...
	/** Cold session activation ends stored waits; attaching to a live runtime must not call this. */
	interruptSessionForResume(sessionId: string): number {
		try {
			return this.#write(() => {
				const normalizedSessionId = identity(sessionId, "sessionId");
				...
				`).get(normalizedSessionId) as { readonly owner_id: unknown } | undefined;
				if (lease?.owner_id !== this.#ownerId) throw new SessionInUseError();
				this.validateRecoveryReferences(normalizedSessionId);
				const rows = this.#database.prepare(`
					SELECT ${RUNTIME_TURN_COLUMNS} FROM runtime_turns
					WHERE session_id = ? AND status = 'in_progress'
					ORDER BY started_at, client_turn_id
				`).all(normalizedSessionId) as readonly RuntimeTurnRow[];
				const rawEffect = this.#stateRepository.loadState(normalizedSessionId, "node_effect_checkpoint");
				const effect = rawEffect === undefined ? undefined
					: parseRuntimeState({ kind: "effect_checkpoint", version: 1, payload: rawEffect });
				for (const row of rows) {
					const turn = runtimeTurnFromRow(row);
					const completedAt = this.#clock();
					if (effect?.kind === "effect_checkpoint" && effect.payload.status === "executing"
						&& effect.payload.turn_id === turn.turn_id
						&& effect.payload.client_turn_id === turn.client_turn_id) {
						this.interruptAmbiguousApproval({
							sessionId: normalizedSessionId,
							clientTurnId: turn.client_turn_id,
							callId: effect.payload.call_id,
							toolName: effect.payload.tool_name,
				...
				this.#database.prepare(`
					DELETE FROM session_state WHERE session_id = ? AND state_key IN (
						'pending_decision', 'suspended_turn', 'node_effect_checkpoint', 'turn_record'
					)
	...
	compareAndSetState(input: CompareAndSetStateInput): boolean {
		if (input.payload !== undefined) this.#validateStandaloneStateReference(input.sessionId, input.key, input.payload);
		return this.#stateRepository.compareAndSetState(input);
	}
	...
	saveParallelApprovalBatch(input: SaveParallelApprovalBatchInput): void {
		...
		const batch = payload.parallel_batch;
		if (!batch) throw new SessionStateError("session_state_invalid", "suspended_turn");
		const clientTurnId = identity(payload.client_turn_id, "clientTurnId");
		const turnId = identity(payload.turn_id, "turnId");
		if (this.#requireRunningTurn(input.sessionId, clientTurnId).turn_id !== turnId) {
			throw new SessionStateError("session_state_invalid", "suspended_turn");
		}
		this.#validateParallelBatchReference(input.sessionId, input.suspendedTurn);
		this.#stateRepository.saveParallelApprovalBatch({
			...input,
			suspendedTurn: {
				...input.suspendedTurn,
				payload: {
					...payload,
					conversation: [],
					transcript_event_id: this.#toolCallEventId(input.sessionId, turnId, batch.calls[0]!.call.callId),
				},
			},
		});
	}
	...
	clearParallelApprovalBatch(sessionId: string, turnId: string, batchId: string): void {
		this.#stateRepository.clearParallelApprovalBatch(sessionId, turnId, batchId);
	}
	...
	upsertShellSnapshot(input: UpsertShellSnapshotInput): void {
		try {
			this.#write(() => {
				...
				const turnId = input.turnId === undefined ? callId : identity(input.turnId, "turnId");
	...
	commitCompaction(input: CommitCompactionInput): boolean {
		try {
			return this.#write(() => {
				const sessionId = identity(input.sessionId, "sessionId");
				if (input.expectedCheckpoint
					&& stableJson(this.loadState(sessionId, "compact_checkpoint"))
	...
	loadReadableTranscriptPage(
		...
		try {
			...
			const eventStart = this.#readableEventStarts(normalizedSessionId);
			...
			while (visibleItemCount(visibleGroups) < limit && !exhausted) {
				...
				const grouped = completeEventGroups(events, pending, eventStart);
	...
	#insertEvent(input: TranscriptEventAppendInput): TranscriptEventEnvelope {
		if (this.errorContextVersion !== 1 && (
			...
			|| (input.eventType === "tool_result" && input.payload.metadata?.error_context !== undefined)
	...
	#validateSuspendedReference(sessionId: string, payload: unknown): void {
		...
		if (state.payload.parallel_batch) {
			this.#validateParallelBatchReference(sessionId, state);
			const head = state.payload.parallel_batch.calls.find((entry) => entry.approval && entry.choice === undefined);
			const pending = this.#stateRepository.loadState(sessionId, "pending_decision");
			if (stableJson(pending ?? null) !== stableJson(head?.approval ?? null)) {
		...
		const rawEffect = this.#stateRepository.loadState(sessionId, "node_effect_checkpoint");
		const effect = rawEffect === undefined ? undefined
			: parseRuntimeState({ kind: "effect_checkpoint", version: 1, payload: rawEffect });
		const resultCommitted = effect?.kind === "effect_checkpoint"
			&& effect.payload.status === "completed"
			&& effect.payload.client_turn_id === clientTurnId
			&& effect.payload.turn_id === turnId
			&& effect.payload.call_id === pending.payload.tool_call.call_id;
		this.#assertContinuationToolCall(
			sessionId,
			clientTurnId,
			turnId,
	...
	#validateParallelBatchReference(
		sessionId: string,
		state: Extract<RuntimeStateRecord, { kind: "suspended_turn" }>,
	): void {
		const batch = state.payload.parallel_batch!;
		const turnId = identity(state.payload.turn_id, "turnId");
		const source = this.#turnEvents(sessionId, turnId).find((event) => (
			event.eventType === "assistant_tool_call_batch"
			&& event.payload.calls.some((call) => call.callId === batch.batch_id)
		));
		if (source?.eventType !== "assistant_tool_call_batch") {
			throw new SessionStateError("session_state_invalid", "suspended_turn");
		}
		const offset = source.payload.calls.findIndex((call) => call.callId === batch.batch_id);
		for (const [index, entry] of batch.calls.entries()) {
			const canonical = source.payload.calls[offset + index];
			if (!canonical || canonical.callId !== entry.call.callId || canonical.name !== entry.call.name
				|| canonical.argumentsJson !== stableJson(toolArguments(entry.call.argumentsJson))) {
				throw new SessionStateError("session_state_invalid", "suspended_turn");
			}
		}
	}
	...
	#readableEventStarts(sessionId: string): ReadableEventStart {
		const turnStarts = new Map<string, number>();
		const callStarts = new Map<string, number>();
		const firstTurnEvent = this.#database.prepare(`
			SELECT sequence_no FROM transcript_events
			WHERE session_id = ? AND turn_id = ?
			ORDER BY sequence_no LIMIT 1
		`);
		const firstCallEvent = this.#database.prepare(`
			SELECT sequence_no FROM transcript_events
			WHERE session_id = ? AND event_type = 'assistant_tool_call_batch'
			AND EXISTS (
				SELECT 1 FROM json_each(payload_json, '$.payload.calls') AS calls
				WHERE json_extract(calls.value, '$.callId') = ?
			)
			ORDER BY sequence_no LIMIT 1
		`);
		return (event) => {
			let start = event.sequenceNo;
			if (event.turnId) {
				if (!turnStarts.has(event.turnId)) {
					const row = firstTurnEvent.get(sessionId, event.turnId) as { sequence_no: number } | undefined;
					turnStarts.set(event.turnId, row?.sequence_no ?? start);
				}
				start = Math.min(start, turnStarts.get(event.turnId)!);
			}
			const callId = callScopedShellId(event);
			if (callId) {
				if (!callStarts.has(callId)) {
					const row = firstCallEvent.get(sessionId, callId) as { sequence_no: number } | undefined;
					callStarts.set(callId, row?.sequence_no ?? start);
				}
				start = Math.min(start, callStarts.get(callId)!);
			}
			return start;
		};
	}
...
type ReadableEventStart = (event: TranscriptEventEnvelope) => number;
...
function readablePageFromEvents(
	...
	const descending = [...events]
		...
		.reverse();
	const eventStart = readableEventStartsFromEvents(events);
	let offset = 0;
	let pending: EventGroup | undefined;
	const visibleGroups: VisibleEventGroup[] = [];
	while (visibleItemCount(visibleGroups) < limit && offset < descending.length) {
		...
		const grouped = completeEventGroups(window, pending, eventStart);
		pending = grouped.pending;
...
function initializeTranscriptSchema(
	...
	try {
		...
		database.exec(version >= SCHEMA_V12_VERSION
			? version >= SCHEMA_V13_VERSION ? SCHEMA_V13_SQL : SCHEMA_V12_SQL
...
function completeEventGroups(
	...
	eventStart: ReadableEventStart,
): { complete: EventGroup[]; pending: EventGroup | undefined } {
	const complete: EventGroup[] = [];
	let current = pending;
	for (const event of events) {
		const firstSequence = eventStart(event);
		// Keep overlapping turn intervals together until their originating inputs/calls are loaded.
		if (current && (event.sequenceNo >= current.firstSequence || sameEventGroup(current, event))) {
			current.events.push(event);
			addEventGroupIdentity(current, event);
			current.firstSequence = Math.min(current.firstSequence, firstSequence);
...
function readableEventStartsFromEvents(events: readonly TranscriptEventEnvelope[]): ReadableEventStart {
	const turnStarts = new Map<string, number>();
	const callStarts = new Map<string, number>();
	for (const event of events) {
		const turnKey = JSON.stringify([event.sessionId, event.turnId]);
		if (event.turnId && !turnStarts.has(turnKey)) turnStarts.set(turnKey, event.sequenceNo);
		if (event.eventType !== "assistant_tool_call_batch") continue;
		for (const call of event.payload.calls) {
			const callKey = JSON.stringify([event.sessionId, call.callId]);
			if (!callStarts.has(callKey)) callStarts.set(callKey, event.sequenceNo);
		}
	}
	return (event) => Math.min(
		event.sequenceNo,
		turnStarts.get(JSON.stringify([event.sessionId, event.turnId])) ?? event.sequenceNo,
		callStarts.get(JSON.stringify([event.sessionId, callScopedShellId(event)])) ?? event.sequenceNo,
	);
}

function callScopedShellId(event: TranscriptEventEnvelope): string | undefined {
	if (event.eventType !== "display_activity" || event.payload.activityType !== "shell") return undefined;
	const callId = event.payload.callId;
	return callId && (!event.turnId || event.turnId === callId) ? callId : undefined;
}
