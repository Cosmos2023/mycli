...
export function projectTranscript(
	...
	const turnsByItemId = new Map<string, string>();
	const createdAtByItemId = new Map<string, string>();
	const interruptedNoticeTurns = new Map<string, string>();
	...
	for (const [index, rawItem] of readableHistory.entries()) {
		...
		turnsByItemId.set(item.id, item.turnId);
		const createdAt = stringValue(item.metadata.created_at);
		if (createdAt) createdAtByItemId.set(item.id, createdAt);
		if (item.type === "warning" && (
			item.metadata.event_kind === "turn_interrupted"
			|| item.id === turnInterruptedNoticeId(item.turnId)
		)) {
			interruptedNoticeTurns.set(item.id, item.turnId);
		}
		if (item.type === "tool_call") {
			const preamble = legacyNodeToolPreamble(item);
			if (preamble && !sameLegacyPreamble(legacyPreamble, preamble)) {
				const assistant = legacyToolPreambleItem(item);
				projected.push(assistant);
				turnsByItemId.set(assistant.id, item.turnId);
				if (createdAt) createdAtByItemId.set(assistant.id, createdAt);
			}
	...
	const ordered = orderInterruptedNotices(projected, turnsByItemId, interruptedNoticeTurns);
	const before = boundedBefore(options.before, ordered.length);
	const limit = boundedLimit(options.limit, ordered.length);
	return Object.freeze(ordered.slice(Math.max(0, before - limit), before).map((item) => {
		const turnId = turnsByItemId.get(item.id);
		const createdAt = item.created_at ?? createdAtByItemId.get(item.id);
		return freezeItem({
			...item,
			...(turnId && turnId !== "unknown-turn" ? { turn_id: turnId } : {}),
			...(createdAt ? { created_at: createdAt } : {}),
		});
	}));
}

function orderInterruptedNotices(
	items: readonly TranscriptItem[],
	turnsByItemId: ReadonlyMap<string, string>,
	interruptedNoticeTurns: ReadonlyMap<string, string>,
): readonly TranscriptItem[] {
	if (interruptedNoticeTurns.size === 0) return items;
	const lastContentByTurn = new Map<string, number>();
	for (const [index, item] of items.entries()) {
		const turnId = turnsByItemId.get(item.id);
		if (turnId && !interruptedNoticeTurns.has(item.id)) lastContentByTurn.set(turnId, index);
	}
	const noticesByAnchor = new Map<number, TranscriptItem>();
	for (const item of items) {
		const turnId = interruptedNoticeTurns.get(item.id);
		const anchor = turnId ? lastContentByTurn.get(turnId) : undefined;
		if (anchor !== undefined) noticesByAnchor.set(anchor, item);
	}
	// Recovery can append several terminal notices after newer inputs. Anchor them after tool merging.
	return items.flatMap((item, index) => {
		const turnId = interruptedNoticeTurns.get(item.id);
		if (turnId && lastContentByTurn.has(turnId)) return [];
		const notice = noticesByAnchor.get(index);
		return notice ? [item, notice] : [item];
	});
}
...
export function sanitizeTranscriptItem(value: unknown): TranscriptItem | undefined {
	...
	const turnId = boundedIdentity(raw.turn_id, 512);
	...
	const metadata = raw.type === "plan_update"
		...
		: raw.type === "clarification"
			...
			: raw.type === "error" || raw.type === "warning"
...
function suppressItem(item: ParsedHistoryItem, approvalTurns: ReadonlySet<string>): boolean {
	...
	if (item.metadata.event_kind === "compaction_model" && item.metadata.readable === false) return true;
...
function visibleMetadata(metadata: Readonly<Record<string, unknown>>): Readonly<Record<string, unknown>> {
	...
	for (const key of ["actualStartLine", "actualEndLine", "shownLines", "totalLines", "rows"] as const) {
		const value = safeInteger(metadata[key]);
		if (value !== undefined && value >= 0) visible[key] = value;
	}
	if (typeof metadata.dedup === "boolean") visible.dedup = metadata.dedup;
...
function visibleErrorMetadata(
	...
	if (visible.code || visible.error_context) {
