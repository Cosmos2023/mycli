...
export function normalizeProviderRequest(value: ProviderRequest): ProviderRequest {
	...
	assertKeys(record, [
		...
		"previousResponseId", "nativeTransport",
...
function normalizeProviderConfig(value: unknown, allowRequestFields = false): ProviderRequestConfig {
	...
	if (!allowRequestFields) {
		for (const key of Object.keys(record)) {
			if (!["provider", "protocol", "model", "nativeTransport", "reasoningEffort", "maxOutputTokens", "sessionId", "cacheRetention", "store", "promptCacheKey", "cacheControlEnabled", "webSearchMode"].includes(key)) {
	...
	const cacheRetention = record.cacheRetention
		?? (record.cacheControlEnabled === false
			...
			: record.promptCacheKey !== undefined || record.cacheControlEnabled === true
				? "short"
				: undefined);
	let nativeTransport: ProviderRequestConfig["nativeTransport"];
	if (record.nativeTransport !== undefined) {
		try { nativeTransport = parseProviderNativeTransportSnapshot(record.nativeTransport); }
		catch { throw invalid("provider native transport snapshot is invalid"); }
		if (nativeTransport.modelId !== record.model || providerNativeProtocol(nativeTransport.api) !== record.protocol) {
			throw invalid("provider native transport does not match request");
		}
	}
	return Object.freeze({
...
function providerConfigFromRequest(request: ProviderRequest): ProviderRequestConfig {
	return Object.freeze({
		...
		model: request.model,
		...(request.nativeTransport ? { nativeTransport: request.nativeTransport } : {}),
...
function canonicalImages(value: unknown): readonly {
	...
	const images = Object.freeze(value.map((image) => {
		const record = normalizedRecord(image, "canonical provider image");
		assertKeys(record, ["mediaType", "data", "detail"], ["mediaType", "data"], "canonical provider image");
		...
		if (record.detail !== undefined && record.detail !== "high" && record.detail !== "original") {
			throw invalid("canonical provider image detail is invalid");
		}
		return Object.freeze({ mediaType: record.mediaType, data,
			...(record.detail === undefined ? {} : { detail: record.detail }) });
	}));
	try { return normalizeCanonicalImages(images); } catch { throw invalid("canonical provider images are invalid"); }
}
