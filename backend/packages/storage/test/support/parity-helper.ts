...
export function expectedSharedRecords(fixture: SessionFixture): unknown[] {
	return fixture.sessions.map((scenario) => {
		...
		if (scenario.status === "completed") {
			...
			const marker = turnAbortedContextItem(scenario.turn_id);
			const metadata = {
				kind: marker.item.metadata.kind,
				role: marker.item.metadata.role,
				cache_class: marker.item.metadata.cacheClass,
				durability: marker.item.metadata.durability,
				scope: marker.item.metadata.scope,
				source_id: marker.item.metadata.sourceId,
				content_sha256: marker.item.metadata.contentSha256,
				content_length: marker.item.metadata.contentLength,
			};
			rawConversation.push({
				role: "context",
				content: marker.item.text,
				tool_call_id: null,
				response_id: null,
