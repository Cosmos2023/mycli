...
test("commits queued images into canonical conversation history", async (t) => {
	const fixture = await databaseFixture(t);
	const store = createStore(fixture.dbPath);
	t.after(() => store.close());
	store.reserveTurn(submission(fixture.root, "s1", NOW));
	const imagePath = "/tmp/queued-image.png";
	const payload = queuePayload();
	store.saveState({
		sessionId: "s1",
		workspaceRoot: fixture.root,
		...
		payload: {
			...payload,
			pending_steers: [{ ...payload.pending_steers[0]!, image_paths: [imagePath] }],
		},
	});
	const record = { ...queuedRecord(), imagePaths: [imagePath] };

	store.commitQueuedInputs({
		sessionId: "s1",
		turnId: "turn-s1",
		records: [record],
		imagesByQueueId: new Map([[
			"q1",
...
test("commits compact replacement, summary, checkpoint, and continuation together", async (t) => {
	...
	).get("s1") as { count: number }).count, 2);
	assert.equal((database.prepare(
		"SELECT COUNT(*) AS count FROM conversation_messages WHERE session_id = ?",
	).get("s1") as { count: number }).count, 1);
	const boundaryRow = database.prepare(`
		SELECT payload_json
		FROM history_items
		WHERE session_id = ? AND json_extract(payload_json, '$.type') = 'compaction_boundary'
	`).get("s1") as { payload_json: string };
	const boundary = JSON.parse(boundaryRow.payload_json) as Record<string, unknown>;
	assert.equal(boundary.source_message_count, 1);
	assert.equal(boundary.summary, "summary");
