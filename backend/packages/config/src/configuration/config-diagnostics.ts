...
import { createErrorContext, type ErrorContext } from "@mycli/contracts";
...
export class ConfigError extends Error {
	...
	constructor(diagnostic: ConfigDiagnostic) {
		...
		this.errorContext = createErrorContext({ reason: "config.invalid", source: "config",
			scope: { kind: "application", id: "configuration" }, outcome: { state: "not_started", effects: "none" },
			details: { ...(diagnostic.keyPath ? { setting: diagnostic.keyPath } : {}) },
		});
	}
}
