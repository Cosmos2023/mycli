...
import { readApiKey, readAuthStore } from "../providers/auth-store.ts";
import { parseProviderRetryOverrides } from "../providers/provider-retry-policy.ts";
import {
	configError,
	isConfigError,
...
async function resolveConfigFromSources(
	...
	const nativeAzure = provider === "azure-openai-responses";
	...
	if (!stableProvider && !nativeAzure && configuredProtocol === undefined) {
	...
	const protocolValue = configuredProtocol ?? (nativeAzure ? "responses" : initialProfile!.defaultProtocol);
	...
	if (!stableProvider && !nativeAzure && configuredModel === undefined) {
		throw invalidConfigValue(
			"model",
	...
	const authStore = await readAuthStore(options.homeDir);
	const storedCredential = authStore.state === "valid" ? authStore.payload[authRef] : undefined;
	const hasStoredOAuth = isRecord(storedCredential) && storedCredential.type === "oauth";
	const apiKey = stringValue(firstTruthy(
		options.env.MYCLI_API_KEY,
		storedApiKey,
		...(hasStoredOAuth ? [] : sources.map((source) => source.api_key)),
	))?.trim() || undefined;
	...
	const requestMaxRetriesByProvider = parseProviderRetryOverrides(
		sources.map((source) => source.request_max_retries_by_provider).find((value) => value !== undefined),
		"request.request_max_retries_by_provider",
	);
	const streamMaxRetriesByProvider = parseProviderRetryOverrides(
		sources.map((source) => source.stream_max_retries_by_provider).find((value) => value !== undefined),
	...
	return {
		...
		...(apiKey ? { apiKey } : {}),
		authRef,
		allowAmbientAuth: options.overrides?.authRef === undefined && options.env.MYCLI_AUTH_REF === undefined
			&& sources.every((source) => source.auth_ref === undefined),
		...
		requestMaxRetries,
		streamMaxRetries,
		requestMaxRetriesByProvider,
		streamMaxRetriesByProvider,
		reasoningEffort,
		thinkingEnabled,
...
function azureEnvironmentBaseUrl(env: Readonly<NodeJS.ProcessEnv>): string | undefined {
	if (env.AZURE_OPENAI_BASE_URL?.trim()) return env.AZURE_OPENAI_BASE_URL.trim();
	const resource = env.AZURE_OPENAI_RESOURCE_NAME?.trim();
	if (!resource) return undefined;
	if (!/^[A-Za-z0-9][A-Za-z0-9-]{0,62}$/u.test(resource)) {
		throw invalidConfigValue("api_base_url", "Azure resource name is invalid", "Set AZURE_OPENAI_BASE_URL or a valid Azure resource name.");
	}
	return new URL("/openai/v1", `https://${resource}.openai.azure.com`).toString();
}
