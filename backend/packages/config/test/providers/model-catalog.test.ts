...
test("SDK output ceilings covering the whole context do not produce a zero input budget", () => {
	const base = { ...BUILTIN_MODEL_CATALOG[0]!, contextWindowTokens: 262_144 };
	assert.equal(modelInputTokenLimit({ ...base, maxOutputTokens: 262_144 }), 262_144);
	assert.equal(modelInputTokenLimit({ ...base, maxOutputTokens: 384_000 }), 262_144);
	assert.equal(modelInputTokenLimit({ ...base, maxOutputTokens: 65_536 }), 196_608);
	assert.equal(modelInputTokenLimit({ ...base, maxOutputTokens: undefined }), 262_144);
	assert.equal(modelInputTokenLimit({ ...base, contextWindowTokens: undefined }), undefined);
});
