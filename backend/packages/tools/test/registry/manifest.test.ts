...
test("exposure planner preserves manifest order and provider schemas", () => {
	...
	assert.equal("description" in shellProperties, false);
	assert.deepEqual(exposure.find((tool) => tool.name === "Shell")?.inputSchema, shell.inputSchema);
	assert.deepEqual(shellProperties.justification, {
		type: "string",
		minLength: 1,
		maxLength: 512,
