...
test("process resources survive yield and close on natural completion", async (t) => {
	const factory = new FakeShellTransportFactory();
	const manager = new ShellSessionManager({ transportFactory: factory.create });
	t.after(async () => { await manager.close(); });
	let closed = false;
	const result = await manager.start(shellStart({
		yieldTimeMs: 0, processResource: { close: async () => { closed = true; } },
	}));
	assert.equal(result.yielded, true);
	assert.equal(closed, false);
	factory.transports[0]!.emitExit({ exitCode: 0, signal: null });
	await eventually(() => closed);
});

test("process resources are revoked on timeout, abort, shutdown and inconclusive cleanup", async (t) => {
	for (const mode of ["timeout", "abort", "shutdown", "inconclusive"] as const) {
		await t.test(mode, async () => {
			const transport = new FakeShellTransport({ kind: "pipe" });
			if (mode === "inconclusive") transport.terminate = async () => ({ state: "inconclusive" });
			const manager = new ShellSessionManager({ transportFactory: async () => transport });
			let closed = false;
			...
			if (mode === "inconclusive") {
				const result = await manager.terminate("session-a", manager.list("session-a")[0]!.shellId);
				assert.equal(result.errorKind, "shell_cleanup_failed");
			}
			if (mode === "shutdown") await manager.close();
			await eventually(() => closed);
			await pending;
			await manager.close();
		});
...
test("failed starts, validation, capacity rejection and a closed manager release resources", async () => {
	const failed = new ShellSessionManager({ transportFactory: async () => { throw new Error("spawn failed"); } });
	const closed = new ShellSessionManager();
	await closed.close();
	const full = new ShellSessionManager({ maxSessions: 1, transportFactory: async () => new FakeShellTransport() });
	await full.start(shellStart({ background: true }));
	for (const manager of [failed, closed, full]) {
		let released = false;
		const result = await manager.start(shellStart({ processResource: { close: async () => { released = true; } } }));
		assert.equal(result.success, false);
		assert.equal(released, true);
		await manager.close();
	}
	let released = false;
	await assert.rejects(closed.start(shellStart({
		callId: "", processResource: { close: async () => { released = true; } },
	})), RangeError);
	assert.equal(released, true);
});

test("shutdown waits for process resource cleanup after an in-flight spawn fails", async () => {
	let rejectStart: (error: Error) => void = () => undefined;
	let releaseResource: () => void = () => undefined;
	let closingResource = false;
	const resourceClosed = new Promise<void>((resolve) => { releaseResource = resolve; });
	const manager = new ShellSessionManager({
		transportFactory: () => new Promise((_resolve, reject) => { rejectStart = reject; }),
	});
	const starting = manager.start(shellStart({ processResource: { close: () => {
		closingResource = true; return resourceClosed;
	} } }));
	await delay(2);
	let finished = false;
	const closing = manager.close().then(() => { finished = true; });
	rejectStart(new Error("spawn failed"));
	await eventually(() => closingResource);
...
test("a yielded process retains its originating turn through output and completion", async (t) => {
	const factory = new FakeShellTransportFactory();
	const events: ShellLifecycleEvent[] = [];
	const manager = new ShellSessionManager({ transportFactory: factory.create });
	t.after(async () => { await manager.close(); });
	const result = await manager.start(shellStart({
		ownerTurnId: "turn-origin", yieldTimeMs: 0,
		publishLifecycle: (event) => { events.push(event); },
	}));
	assert.equal(result.processState, "running_background");
	const transport = factory.transports[0]!;
	transport.emitOutput({ sequence: 1, stream: "stdout", data: "finished" });
	transport.emitExit({ exitCode: 0, signal: null });
	await eventually(() => events.some((event) => event.kind === "shell.completed"));
	assert.ok(events.some((event) => event.kind === "shell.output"));
	assert.ok(events.every((event) => event.ownerTurnId === "turn-origin"));
});

test("zero-wait startup failure returns no running handle or lifecycle start", async () => {
	const events: ShellLifecycleEvent[] = [];
	const manager = new ShellSessionManager({
		transportFactory: async () => { throw new Error("spawn failed"); },
	});
	const result = await manager.start(shellStart({
		yieldTimeMs: 0, publishLifecycle: (event) => { events.push(event); },
	}));
	assert.equal(result.success, false);
	assert.equal(result.errorKind, "shell_spawn_failed");
	assert.equal(result.shellId, "");
