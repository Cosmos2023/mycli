...
test("full access reads and patches an outside file with the shared snapshot guard", async (t) => {
	const fixture = await workspaceFixture(t);
	...
	await adapters.read.execute({ file_path: outside, offset: 1, limit: 20 }, fullAccess);
