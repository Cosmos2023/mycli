...
export class FileHistoryStore {
	...
	}): Promise<readonly FileHistorySnapshot[]> {
		const events = await this.#events(sessionDigest(input.sessionId));
		const terminal = new Set(events
			.filter((event) => event.action === "discard" || event.action === "undo")
			.map((event) => event.snapshot_id));
		const seen = new Set<string>();
		const snapshots: FileHistorySnapshot[] = [];
		const limit = Math.max(0, Math.min(input.limit ?? 10, 100));
		for (let index = events.length - 1; index >= 0 && snapshots.length < limit; index -= 1) {
			const event = events[index]!;
			if (event.action !== "before" || terminal.has(event.snapshot_id) || seen.has(event.snapshot_id)) {
				continue;
			}
			seen.add(event.snapshot_id);
