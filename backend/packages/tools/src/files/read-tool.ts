...
import { READ_TOOL_DEFINITION } from "../registry/manifest.ts";
import {
...
export class ReadTool implements ToolAdapter {
	...
	async execute(
		...
		try {
			const allowedRoots = options.executionPolicy
				? Object.freeze([
					...(options.executionPolicy.readableRoots ?? []),
					...options.executionPolicy.writableRoots,
				])
				: undefined;
