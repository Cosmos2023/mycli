...
export class ToolRouter implements ToolRouterContract {
	...
	async execute(
		...
		const route = this.#routesFor(options.ownerTurnId).get(call.name);
		if (!route) {
			return failure(call, "unknown_tool", "Tool is not available.", options);
		}
		const argumentsValue = parseArguments(call.argumentsJson);
		if (!argumentsValue || !route.validate(argumentsValue)) {
			return failure(call, "invalid_arguments", "Invalid tool arguments.", options);
		}
		const result = await route.adapter.execute(argumentsValue, options);
		let images;
		try {
			images = result.images === undefined ? undefined : normalizeCanonicalImages(result.images);
		} catch {
			return failure(call, "invalid_image", "Tool returned invalid or oversized image data.", options, true);
		}
		const errorContext = toolErrorContext(call, result, options);
		const normalizedResult = { ...result };
		delete normalizedResult.errorContext;
	...
	#compileRoutes(
		...
		for (const adapter of adapters) {
			...
			routes.set(name, {
				adapter,
				validate: this.#ajv.compile(adapter.legacyInputSchemas?.length
					? { anyOf: [adapter.definition.inputSchema, ...adapter.legacyInputSchemas] }
					: adapter.definition.inputSchema),
			});
		}
		return routes;
	}
...
function failure(
	...
	const errorContext = toolErrorContext(call, result, options, dispatched);
	return { ...result, ...(errorContext ? { errorContext, metadata: { error_context: errorContext } } : {}) };
}
