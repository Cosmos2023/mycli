...
import {
	...
	resolvePublicTarget,
	PublicTargetError,
	type PublicTargetLookup as WebFetchLookup,
} from "./network/public-target.ts";

import { WEB_FETCH_TOOL_DEFINITION } from "./registry/manifest.ts";
...
export { normalizePublicUrl, resolvePublicTarget, isPublicIpAddress } from "./network/public-target.ts";
export type { PublicTargetLookup as WebFetchLookup } from "./network/public-target.ts";
	...
	async #followRedirects(
		...
		for (let redirects = 0; ; redirects += 1) {
			...
			try {
				...
				if (error instanceof PublicTargetError && error.kind === "unsafe_address") {
...
export class PinnedPublicWebFetcher implements PublicWebFetcher {
	readonly #maxResponseBytes: number;
	readonly #lookup: WebFetchLookup | undefined;
...
function failureFrom(error: unknown, fallbackKind: string): ToolAdapterResult {
	return error instanceof WebFetchFailure || error instanceof PublicTargetError
