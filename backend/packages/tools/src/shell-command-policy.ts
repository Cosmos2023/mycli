...
const POWERSHELL_DIRECT_SAFE = new Set([
	"get-childitem", "get-command", "get-content", "get-date", "get-location", "test-path",
]);
const POWERSHELL_PIPELINE_SAFE = new Set(["measure-object", "select-object", "sort-object"]);
...
export interface ShellCommandClassification {
	readonly decision: "safe" | "unknown" | "complex" | "invalid";
...
export function classifyShellCommand(
	command: string,
	options: { readonly shellKind: ShellCommandKind; readonly platform?: NodeJS.Platform },
...
export function isKnownSafeShellSegment(
	...
	if (executable === "git") return safeGit(value.words);
...
function isKnownSafeWindowsSegment(value: ShellSegment, shellKind: "powershell" | "cmd"): boolean {
	...
	if (shellKind === "powershell") {
		return POWERSHELL_DIRECT_SAFE.has(executable)
