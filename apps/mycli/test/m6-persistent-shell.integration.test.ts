...
test("Node backend runs one approved PTY through Shell and WriteStdin without Python", async (t) => {
	...
	const approval = await waitFor(() => event(messages, "approval.request"));
	...
	await request(backend, messages, "approve", "approval.respond", {
...
test("M6 live smoke drives an approved PTY and reports only structural state", async (t) => {
