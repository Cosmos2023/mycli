...
test("extension doctor isolates malformed trees, reports migration, and closes probes", {
	...
	await mkdir(join(mycliRoot, "subagents"), { recursive: true });
	...
	await writeFile(join(mycliRoot, "subagents", "broken.toml"), [
		'id = "broken"',
		'description = "profile-test-secret"',
		'instruction = "private profile prompt"',
		"[budget]",
		"max_turns = 0",
	].join("\n"), "utf8");
	...
	assert.equal(byName.get("subagents")?.status, "warning");
	assert.equal(byName.get("mcp")?.status, "ok");
	assert.equal(pluginCloseCount, 1);
	assert.equal(mcpCloseCount, 1);
	assert.doesNotMatch(
		JSON.stringify(checks),
		/hook-test-secret|skill-test-secret|profile-test-secret|python-test-secret|private.*prompt/u,
	);
});
