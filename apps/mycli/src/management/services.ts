...
export interface SubagentManagementContract {
	list(): MaybePromise<ManagementResponse>;
	inspect(profileId: string): MaybePromise<ManagementResponse>;
}
...
export interface ManagementServicesOptions {
	...
	readonly subagents: SubagentManagementContract;
...
export class ManagementServices implements ManagementExecutor {
	...
	#dispatch(command: ManagementCommand, signal: AbortSignal): MaybePromise<ManagementResponse> {
		...
		return command.action === "list"
			? this.#services.subagents.list()
			: this.#services.subagents.inspect(command.profileId);
	}
}
...
export async function createDefaultManagementServices(
	...
	const registry = await SubagentProfileRegistry.discover(options);
	const subagents = new SubagentManagementService({ registry });
