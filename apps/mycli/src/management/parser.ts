...
const MANAGEMENT_COMMANDS = new Set(["doctor", "setup", "hooks", "plugins", "mcp", "subagents"]);
...
function parseManagementCommand(root: string, rawArgs: readonly string[]): ManagementCommand {
	...
	return parseSubagents(args, json);
}
...
function parseSubagents(args: readonly string[], json: boolean): SubagentsManagementCommand {
	const action = args[0];
	if (action === "list" && args.length === 1) {
		return Object.freeze({ kind: "subagents", action, json });
	}
	if (action === "inspect" && args.length === 2) {
		return Object.freeze({ kind: "subagents", action, profileId: nonEmpty(args[1]), json });
	}
	throw usage("subagents list|inspect [profile_id] [--json]");
}
...
function validateInteractiveArguments(argv: readonly string[]): void {
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (argument === "--session" || argument === "--model" || argument === "--runtime-backend") {
		...
		if (argument?.startsWith("--session=")
			...
			|| argument?.startsWith("--runtime-backend=")) {
