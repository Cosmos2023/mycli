...
**Reference APIs checked:** Textual `App`, `ComposeResult`, `Input`, `RichLog`, `ModalScreen`, `App.run_test()`/`Pilot.press()`, thread workers, and `call_from_thread()` from official Textual docs.
...
In `src/mycli/cli/main.py`, preserve the existing testable `main(..., input_func=..., output_func=...)` signature and add imports:
...
def test_execution_status_label_uses_turn_elapsed_seconds() -> None:
    ...
    assert execution_status_label(phase="editing", elapsed_seconds=44.2) == "Editing files... (44s)"
...
```python
@pytest.mark.asyncio
async def test_tui_enter_runs_turn_in_worker_and_renders_final_answer(tmp_path: Path) -> None:
    class Service(FakeService):
        def handle_user_turn(self, message: str, stream_sink=None) -> TurnResponse:
...
```bash
uv run pytest tests/unit/cli/test_tui_app.py::test_tui_enter_runs_turn_in_worker_and_renders_final_answer tests/unit/cli/test_tui_app.py::test_tui_ctrl_c_restores_pre_send_input tests/unit/cli/test_tui_app.py::test_tui_resume_command_renders_session_lines tests/unit/cli/test_tui_transcript.py::test_final_answer_renderable_uses_markdown -q
```
...
from mycli.cli.tui.transcript import execution_status_label, final_answer_renderable, phase_for_tool
```
...
Add worker-backed turn methods. The worker thread owns the blocking `handle_user_turn()` call; all Textual widget writes return to the app thread through `call_from_thread()`:
