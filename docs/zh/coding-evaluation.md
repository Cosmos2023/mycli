...
打包提示词版本为 `2026-09-codex-style-base-v23`。重新构建、重启 mycli 并创建新会话，才使用更新的基础指令。已有会话保留冻结指令快照，不重写。
...
受支持文件优先用 Read，并显式给出 `offset` 和 `limit`。先定位再读取的规则适用于所有读取方式：先用 `rg -n`、`rg -c` 找到目标区域，再只读取该窗口，不用脚本分块打印整个文件。Read 不可用、操作失败或不支持格式时，可在相同许可范围内使用有界 Shell 读取或适当解析器，同样保持这两步。窗口命令按 shell 区分：PowerShell 用 `Get-Content | Select-Object -Skip/-First`，POSIX shell 用 `sed -n` 或 `rg -n -A/-B`，CMD 用 `rg -n -A/-B` 或短 Node 脚本（Node 始终可用）。限制路径、提取范围和输出大小，不输出无限文件或原始二进制。

每轮环境上下文会报告 `platform`、`shell`、`shell_kind`、`shell_dialect` 和只陈述事实的 `shell_notes`（当前方言名）。写命令的规则改由 `Shell` 工具描述承载，与 Codex 一致：方言语法、固定 UTF-8 编码规则、Windows 安全规则，以及 macOS 的 BSD userland 注意事项（`sed -i` 必须显式给后缀，`date -d`、`readlink -f`、`stat -c`、`xargs -r` 以及 GNU 专有长选项不可用）都写在工具描述里。基础提示词只指向该描述并保留跨平台规则。
