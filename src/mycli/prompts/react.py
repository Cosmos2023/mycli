...
def build_react_prompt() -> str:
    return "\n".join(
        ...
        "搜索文本和找文件优先使用 Bash 跑 rg / rg --files；rg 不可用时再用 Grep、Glob 或系统替代命令。",
        "精读文件、引用证据或准备修改前，用 Read 获取行号、截断和快照元数据。",
        "修改文件必须使用 Edit / Patch / Write，不要用 sed、awk、perl 或 Python 脚本直接改文件。",
